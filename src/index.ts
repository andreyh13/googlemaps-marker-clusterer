...
export function getMarkerClustererClass(parentClass: typeof google.maps.OverlayView) {
  const mClass = class MarkerClusterer extends parentClass {
      ...
      if (!this.pIdleListener) {
        this.pIdleListener = google.maps.event.addListener(this.getMap(), 'idle', () => {
          if (!this.pFirstIdle) {
            ...
            this.pFirstIdle = this.numMarkers === 0;
